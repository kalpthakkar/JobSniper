"""
core/microsoft_poller.py — Dedicated poller for Microsoft Careers

This poller runs independently, fetching jobs from Microsoft Careers API
...
from company.microsoft.microsoft import MicrosoftAdapter, MicrosoftJobDetailFetcher

logger = logging.getLogger("job_sniper.microsoft_poller")

MICROSOFT_DISAPPEARANCE_THRESHOLD = 3  # Remove job if missing from 3 consecutive polls


class MicrosoftPoller:
    def __init__(
        self,
        db: JobDatabase,
        notifier: Notifier,
        ...
        self.adapter = MicrosoftAdapter(timeout=request_timeout)
        self.detail_fetcher = MicrosoftJobDetailFetcher(timeout=request_timeout)
    ...
    def _run_loop(self):
        while not self._stop.is_set():
            try:
                ...
                logger.error(f"[microsoft] Polling cycle error: {e}", exc_info=True)
                time.sleep(60)  # Wait a minute before retrying on error

            if not self._stop.is_set():
                logger.info(
                    f"[microsoft] Cycle complete, cooling down for {self.cooldown_seconds}s"
                )
                self._stop.wait(self.cooldown_seconds)

    def _poll_cycle(self):
        """Poll Microsoft jobs and manage tracking."""
        logger.debug("[microsoft] Polling cycle started")

        # Fetch all recent jobs (last 6 hours)
        all_jobs, total_records = self.adapter.fetch_all_recent_jobs(hours=6)
        if not all_jobs:
            logger.warning("[microsoft] No recent jobs fetched from Microsoft")
            return

        all_ids = sorted({str(job.get("id", "")) for job in all_jobs if job.get("id")})
        logger.info(f"[microsoft] Fetched {len(all_ids)} recent job(s) from {total_records} total")
    ...
    def _process_jobs(self, jobs_list: List[dict], all_ids: List[str]):
        ...
        endpoint = "microsoft"  # Fixed endpoint for Microsoft
        ...
        if record is None:
            ...
            logger.info(
                f"[microsoft] 🌱 FIRST RUN: Baseline set — {len(all_ids)} job(s) recorded. "
                f"Monitoring started. New jobs will be detected and notified from the next poll cycle onwards."
            )
            return

        # Check for changes
        if set(seen_ids) == set(all_ids):
            logger.debug("[microsoft] ✓ Stable job set")
            return
        ...
        for jid in removed_ids_candidates:
            remaining = absent_counts.get(jid, MICROSOFT_DISAPPEARANCE_THRESHOLD)
            remaining -= 1
            if remaining <= 0:
        ...
        if confirmed_removed:
            logger.info(
                f"[microsoft] ➖ {len(confirmed_removed)} job(s) removed: {confirmed_removed}"
            )
    ...
    def _fetch_new_job_details(
        ...
        """Fetch detailed metadata for new Microsoft jobs and build Job objects."""
        jobs = []
        ...
        logger.debug(f"[microsoft] Fetching details for {len(new_ids)} new job(s)")

        # Fetch details for all new jobs in parallel
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {
                executor.submit(self.detail_fetcher.fetch_job_detail, job_id): job_id
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.warning(f"[microsoft] Error fetching details for job {job_id}: {e}")

        logger.debug(
            f"[microsoft] Successfully built {len(jobs)} Job objects from {len(new_ids)} new jobs"
        )
        return jobs

    @staticmethod
    def _build_job_from_details(base_job: dict, job_details: dict) -> Optional[Job]:
        """Build a Job object from Microsoft job base info and details."""
        try:
            job_id = str(base_job.get("id", ""))
            if not job_id:
                return None

            title = base_job.get("name", "Untitled")
            location = MicrosoftAdapter.extract_location(base_job)
            position_url = base_job.get("positionUrl", "")
            url = f"https://apply.careers.microsoft.com{position_url}"
            remote = MicrosoftAdapter.is_remote(base_job)
            department = base_job.get("department", "")

            # Build description from job details
            description = MicrosoftJobDetailFetcher.build_job_description(job_details)
            ...
            logger.warning(f"[microsoft] Failed to build Job object: {e}")
            return None
